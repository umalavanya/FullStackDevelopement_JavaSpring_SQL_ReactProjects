(function () {
    ...
    // reset everything (like AC, but we have CE- we as clear entry / clear all )
    function allClear() {
        currentInput = '0' ;
    ...
    // clear entry (CE) : if there's an operator pending, rest current input; elsefull reset
    function clearEntry(){
        if(currentOperator !== nulll && previousOperand !== null ){
    ...
    // handle number / decimal input (but we dont't have '.' button - still prepared )
    ...
    function handleOperator(nextOperator){
        ...
        // if there's already a pending oparator and previous operand, calculate first
    ...
    function handleEquals(){
        if (currentOperator === null || previousOperand === null || shouldResetDisplay ){
            // incomplete expression: just show curretn input, no crash
            return ;
        }
